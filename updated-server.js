...
app.post('/webhook', async (req, res) => {
    try {
        ...
        // Prepare payload for Discord with better formatting
        const playerData = req.body;
        const isJoinEvent = playerData.event === 'player_joined';
        const isLeaveEvent = playerData.event === 'player_left';
        
        const discordPayload = {
            embeds: [{
                title: isJoinEvent ? '🎮 Player Joined!' : isLeaveEvent ? '👋 Player Left' : 'Roblox Event',
                color: isJoinEvent ? 0x00FF00 : isLeaveEvent ? 0xFF9900 : 0x0099FF,
                fields: [
                    {
                        name: 'Player',
                        value: `**${playerData.username}** (ID: ${playerData.userId})`,
                        inline: true
                    },
                    {
                        name: 'Game',
                        value: playerData.game || 'Unknown Game',
                        inline: true
                    },
                    {
                        name: 'Time',
                        value: playerData.time || new Date().toISOString(),
                        inline: true
                    }
                ],
                timestamp: new Date().toISOString(),
                footer: {
                    text: 'Roblox Webhook System'
                }
            }]
        };
        
        // Add optional fields if they exist
        if (playerData.language) {
        ...
        if (playerData.accountAge !== undefined) {
            discordPayload.embeds[0].fields.push({
                name: 'Account Age',
                value: `${playerData.accountAge} days`,
                inline: true
            });
        }
